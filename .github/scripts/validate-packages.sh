#!/usr/bin/env bash
# Validates the packed NuGet packages before anything is published.
#
# Usage: validate-packages.sh <version> <packages-dir>
# Checks for each of Ethar.GeoPose and Ethar.GeoPose.Authority:
#   - the .nupkg exists and the .snupkg symbols package exists
#   - the nuspec version matches the expected version
#   - readme, icon, licence and the netstandard2.0 and net48 assemblies are inside the package
#   - the Authority package depends on the same Ethar.GeoPose version that is being released
set -euo pipefail
